...
//! and close the record is orphaned in `/run/unix-oidc/sessions/`.
//!
...
pub fn sweep_expired_sessions(session_dir: &Path) {
    ...
    for entry in entries {
        ...
        // the full SessionRecord type from pam-unix-oidc.  This avoids a circular
...
mod tests {
    ...
    // Test 8: env override UNIX_OIDC_TIMEOUTS__SWEEP_INTERVAL_SECS works via figment
    #[test]
    fn test_env_override_sweep_interval() {
        ...
        std::env::set_var("UNIX_OIDC_TIMEOUTS__SWEEP_INTERVAL_SECS", "120");
        // Clear interfering legacy var
        std::env::remove_var("UNIX_OIDC_JWKS_CACHE_TTL");

        let config = AgentConfig::load_from_path(&config_path).unwrap();
        assert_eq!(config.timeouts.sweep_interval_secs, 120);

        std::env::remove_var("UNIX_OIDC_TIMEOUTS__SWEEP_INTERVAL_SECS");
    }
}
