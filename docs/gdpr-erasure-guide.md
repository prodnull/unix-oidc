# GDPR Article 17 Erasure Guide for unix-oidc
...
unix-oidc stores, processes, or logs during normal operation, together with the
...
available within unix-oidc. It does not constitute legal advice. Organizations subject
...
The following table enumerates every data category unix-oidc handles, where it lives,
...
| OIDC refresh tokens | System keyring (Secret Service / macOS Keychain) or `~/.local/share/unix-oidc/` | Until explicitly revoked or agent storage cleared | Yes — bearer credential tied to user identity | Stored via `unix-oidc-agent storage` backend |
...
| DPoP private keys (persisted) | System keyring or `~/.local/share/unix-oidc/` | Until key rotation or storage clear | No | Stored by key thumbprint; no PII in the key itself |
| Session records | `/run/unix-oidc/sessions/{session-id}.json` | Active session lifetime; swept by `SESSION_SWEEP` task | Yes — username, source IP, session start time | Under `/run`; lost on reboot; files are mode 0600 root |
| Audit log entries (file) | `/var/log/unix-oidc-audit.log` | Per logrotate retention policy (default: 1 year) | Yes — username, source IP, issuer URL, timestamps | See `docs/log-retention.md` for compliance periods |
| Audit log entries (syslog) | syslog daemon (varies: `/var/log/auth.log`, journald) | Per syslog/journald retention policy | Yes | Rotation managed by system syslog, not unix-oidc |
| Audit log entries (SIEM) | External SIEM (Splunk, ELK, Datadog, CloudWatch, etc.) | Per SIEM/org retention policy | Yes | Outside unix-oidc's control — see §3 |
| Issuer health state | `/run/unix-oidc/issuer-health/{issuer-hash}.json` | Transient; lost on reboot | No — contains issuer URL hash, failure count, timestamps only | Not linked to any individual user |
| Configuration files | `/etc/unix-oidc/policy.yaml`, `/etc/unix-oidc/config.yaml` | Indefinite — until updated by administrator | Conditionally — break-glass usernames if configured | PII only if operator includes real usernames in break-glass config |

### PII Classification Summary

unix-oidc's primary PII exposure is in **audit log entries** and **session records**.
...
- Both occur when the unix-oidc-agent process terminates or the session ends.
...
**Mechanism:** `unix-oidc-agent storage clear` or manual keyring deletion.

```bash
# Preferred: use the agent's built-in storage management command
unix-oidc-agent storage clear

# Linux — Secret Service (GNOME Keyring / KWallet)
# List items to confirm before deletion:
secret-tool search service unix-oidc
# Delete all unix-oidc items:
secret-tool clear service unix-oidc

# Linux — kernel keyutils (user keyring)
# List items:
keyctl show @u | grep unix-oidc
# Revoke each key by ID:
keyctl revoke <key-id>

# macOS — Keychain
security delete-generic-password -s unix-oidc -a <username>
```

**Verification:**

```bash
# After clearing, confirm no unix-oidc items remain
secret-tool search service unix-oidc   # Linux Secret Service
keyctl show @u                          # Linux keyutils
security find-generic-password -s unix-oidc  # macOS
```

#### File-Based Storage (`~/.local/share/unix-oidc/`)

**Mechanism:** `unix-oidc-agent storage clear` invokes a three-pass secure overwrite
...
shred -u ~/.local/share/unix-oidc/token
shred -u ~/.local/share/unix-oidc/dpop-key

# Or remove the entire directory
shred -u ~/.local/share/unix-oidc/*
rmdir ~/.local/share/unix-oidc/
```
...
blocks. unix-oidc logs a WARNING at startup if CoW or SSD is detected. The authoritative
...
#### Session Records (`/run/unix-oidc/sessions/`)
...
#### Audit Logs (Local File: `/var/log/unix-oidc-audit.log`)

**Mechanism:** Secure deletion of the audit log file plus any rotated copies.

```bash
# Erase the active audit log and all rotated copies
shred -u /var/log/unix-oidc-audit.log
shred -u /var/log/unix-oidc-audit.log.1
shred -u /var/log/unix-oidc-audit.log.*.gz  # compressed rotations

# Or, for the full unix-oidc log directory:
find /var/log/unix-oidc/ -type f | xargs shred -u
```
...
jq -c 'select(.username != "alice")' \
  /var/log/unix-oidc-audit.log > /tmp/audit-filtered.log

# Step 2: Verify the filtered file looks correct
wc -l /var/log/unix-oidc-audit.log /tmp/audit-filtered.log

# Step 3: Atomically replace with shred of original
shred -u /var/log/unix-oidc-audit.log
mv /tmp/audit-filtered.log /var/log/unix-oidc-audit.log
chmod 0640 /var/log/unix-oidc-audit.log
...
to file-based audit logging (unix-oidc `audit.output: file`) to enable the targeted
erasure procedure above.

#### Configuration Files (`/etc/unix-oidc/`)
...
## Section 3: What Cannot Be Erased by unix-oidc

The following data categories are outside unix-oidc's control once they leave the
...
other external system cannot be deleted or modified by unix-oidc. The data controller
must:
...
independently of unix-oidc's own audit log. Erasure from auditd requires:
...
controlled by the IdP administrator, not unix-oidc. To erase IdP-side records:
...
These snapshots are outside unix-oidc's control. To ensure complete erasure:
...
erasure request for a specific user in a unix-oidc deployment.
...
[ ] 2. Disable the user's account (prevent re-authentication during erasure)
        ...
        - Remove from any unix-oidc group/role policies

[ ] 3. Clear agent storage (if the user runs unix-oidc-agent on any endpoint)
...
[ ] 7. Purge backup copies
        - Identify all backup jobs/media containing unix-oidc logs
...
```bash
#!/usr/bin/env bash
# unix-oidc-gdpr-erase.sh
# GDPR Article 17 erasure helper for unix-oidc
# Usage: unix-oidc-gdpr-erase.sh <username>
#
...
LOG_FILE="/var/log/unix-oidc-gdpr-erase-${USERNAME}-${TIMESTAMP}.log"
...
SESSION_FILES=$(grep -rl "\"username\":\"${USERNAME}\"" \
  /run/unix-oidc/sessions/ 2>/dev/null || true)
...
if [[ -f /var/log/unix-oidc-audit.log ]]; then
    ORIG_COUNT=$(wc -l < /var/log/unix-oidc-audit.log)
    jq -c --arg u "${USERNAME}" 'select(.username != $u)' \
      /var/log/unix-oidc-audit.log > /tmp/unix-oidc-audit-filtered.log
    FILTERED_COUNT=$(wc -l < /tmp/unix-oidc-audit-filtered.log)
    REMOVED=$(( ORIG_COUNT - FILTERED_COUNT ))
    echo "    Removed ${REMOVED} log entries for ${USERNAME}" | tee -a "${LOG_FILE}"
    shred -u /var/log/unix-oidc-audit.log
    mv /tmp/unix-oidc-audit-filtered.log /var/log/unix-oidc-audit.log
    chmod 0640 /var/log/unix-oidc-audit.log
    chown root:adm /var/log/unix-oidc-audit.log
    echo "    NOTE: If HMAC tamper-evidence is enabled, the chain is now broken." \
      | tee -a "${LOG_FILE}"
else
    echo "    /var/log/unix-oidc-audit.log not found; skipping." | tee -a "${LOG_FILE}"
fi
...
echo "    unix-oidc-agent storage clear" | tee -a "${LOG_FILE}"
...
In a unix-oidc deployment, this exemption applies when audit logs are retained as
evidence for:
