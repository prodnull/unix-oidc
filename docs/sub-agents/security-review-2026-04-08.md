# Security Review — prmana (2026-04-08)
...
- **Scope**: All production `.rs` files across `pam-prmana`, `prmana-agent`, and `rust-oauth-dpop`
...
- **File**: `prmana-agent/src/storage/file_store.rs:66-79`
...
- **File**: `prmana-agent/src/askpass.rs:74-84`
- **Issue**: `fs::write()` then `set_permissions(0o600)`. Path is `$TMPDIR/.prmana-token-{PPID}` — predictable. Symlink attack vector + no secure delete on read.
...
- **File**: `prmana-agent/src/main.rs:181`
...
- **File**: `pam-prmana/src/sudo.rs:657-664`
...
- **File**: `pam-prmana/src/approval/provider.rs:225-232`
...
- **File**: `pam-prmana/src/approval/webhook.rs:111-116`
- **Issue**: `PRMANA_WEBHOOK_INSECURE=true` disables cert verification without `test-mode` feature gate. Production deployments could be MITM'd.
...
- **File**: `pam-prmana/src/ciba/client.rs:131-137`
...
- **File**: `pam-prmana/src/lib.rs:835`
...
- **File**: `prmana-agent/src/daemon/socket.rs:548-595`
...
- **File**: `pam-prmana/src/security/rate_limit.rs:247-265`
...
- **File**: `pam-prmana/src/security/rate_limit.rs:201`
...
- **File**: `pam-prmana/src/security/rate_limit.rs:96-121`
...
- **File**: `prmana-agent/src/crypto/dpop.rs:162-196`
...
- **Files**: `pam-prmana/src/security/jti_cache.rs`, `nonce_cache.rs`, `rust-oauth-dpop/src/server.rs`
...
- **Future mitigation**: A shared-state cache via the `prmana-agent` daemon (running as a system service) is a candidate for a future milestone. Tracked as a design decision.

### VULN-002: Implicit trust in agent for CIBA step-up (Gemini finding) — MEDIUM (acknowledged)
- **File**: `pam-prmana/src/sudo.rs:371-377`
...
- **Files**: `pam-prmana/src/auth.rs`, `pam-prmana/src/oidc/token.rs`
...
- **Verification**: Confirmed — `grep -r "SecretString\|Zeroiz" pam-prmana/src/` returns zero hits.
- **Design rationale**: The `prmana-agent` (client-side daemon) uses full memory protection (SecretString, mlock, ZeroizeOnDrop) because it holds long-lived keys. The PAM module handles tokens transiently in a short-lived forked process that exits after authentication.
...
| **DPoP thumbprint computation (PAM)** | Hardcoded canonical `"P-256"`/`"EC"` in `pam-prmana/src/oidc/dpop.rs`. Correct per RFC 7638. |
