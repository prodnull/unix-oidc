//! unix-oidc-evidence-export — local/export-first posture snapshots and evidence bundles.

use std::path::PathBuf;
use std::process;

use clap::{Parser, Subcommand, ValueEnum};
use pam_unix_oidc::evidence::{
    build_evidence_export_bundle, load_host_posture_snapshot, parse_rfc3339_to_utc,
    render_evidence_events_csv, EvidenceFilter,
};

#[derive(Parser, Debug)]
#[command(name = "unix-oidc-evidence-export")]
#[command(about = "Generate host posture snapshots and evidence exports from unix-oidc state")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    Posture {
        #[arg(long, default_value = "/etc/unix-oidc/policy.yaml")]
        policy: PathBuf,
    },
    /// Export filtered evidence from the audit log.
    Export {
        #[arg(long, short = 'f', default_value = "/var/log/unix-oidc-audit.log")]
        file: PathBuf,
        #[arg(long)]
