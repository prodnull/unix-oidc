#!/bin/bash
# prmana installer
# Usage: curl -fsSL https://raw.githubusercontent.com/prodnull/prmana/main/deploy/installer/install.sh | bash
set -euo pipefail
...
check_pam_safety() {
    ...
    # Check if sshd already has pam_prmana configured
    if [ -f /etc/pam.d/sshd ]; then
        if grep -q "pam_prmana" /etc/pam.d/sshd 2>/dev/null; then
            sshd_has_oidc=true
            log_warn "sshd already has pam_prmana.so configured"
        fi
    ...
    # Check if sudo already has pam_prmana configured
    if [ -f /etc/pam.d/sudo ]; then
        if grep -q "pam_prmana" /etc/pam.d/sudo 2>/dev/null; then
            sudo_has_oidc=true
            log_warn "sudo already has pam_prmana.so configured"
        fi
        # Check for fallback auth (pam_unix)
    ...
    if [ "$sshd_has_oidc" = true ] && [ "$sshd_has_fallback" = false ]; then
        log_warn "sshd has pam_prmana but no pam_unix fallback!"
    fi
    if [ "$sudo_has_oidc" = true ] && [ "$sudo_has_fallback" = false ]; then
        log_warn "sudo has pam_prmana but no pam_unix fallback!"
    fi
    ...
    if [ "$sshd_has_oidc" = true ] && [ "$sudo_has_oidc" = true ]; then
        if [ "$sshd_has_fallback" = false ] || [ "$sudo_has_fallback" = false ]; then
            log_error "CRITICAL: Both sshd and sudo have pam_prmana configured"
...
download_binary() {
    ...
    local artifact_name="prmana-v${VERSION}-linux-${arch}.tar.gz"
    ...
    if [ -n "$OFFLINE_TARBALL" ]; then
        ...
        log_info "Downloading prmana v${VERSION} for linux-${arch}..."
...
install_pam_module() {
    local tmp_dir="$1"
    local pam_module="$tmp_dir/libpam_prmana.so"
    local dest="$PAM_DIR/pam_prmana.so"
...
install_agent() {
    local tmp_dir="$1"
    local agent="$tmp_dir/prmana-agent"
    local dest="/usr/local/bin/prmana-agent"
...
# /etc/pam.d/sshd - Recommended configuration for prmana
# WARNING: Test thoroughly before applying to production!

# Load environment from prmana config
auth       required     pam_env.so envfile=/etc/prmana/config.env

# Try prmana first, fall back to standard auth
...
# /etc/pam.d/sudo - Recommended configuration for prmana
# WARNING: Test thoroughly before applying to production!
...
create_manifest() {
    ...
    cat > "$manifest" << EOF
# prmana install manifest
# Created: $(date -Iseconds)
# Version: $VERSION
# Installer: $SCRIPT_VERSION

$PAM_DIR/pam_prmana.so
/usr/local/bin/prmana-agent
...
test_with_pamtester() {
    ...
    if pamtest_output=$(timeout 5 pamtester sshd root authenticate 2>&1 </dev/null); then
        ...
        if echo "$pamtest_output" | grep -qi "module.*not found\|cannot.*load\|no such file"; then
            ...
            log_error "Check that $PAM_DIR/pam_prmana.so exists and has correct permissions"
            return 1
    ...
    case "$OS_FAMILY" in
        debian)
            if command -v pam-auth-update &>/dev/null; then
                ...
                cat > /usr/share/pam-configs/prmana << 'PAMCFG'
Name: prmana OIDC Authentication
Default: no
Priority: 192
Auth-Type: Primary
Auth:
    [success=end default=ignore] pam_prmana.so
PAMCFG
                log_success "Installed pam-auth-update profile"
                log_info "Enable with: sudo pam-auth-update --enable prmana"
        ...
        rhel)
            if command -v authselect &>/dev/null; then
                ...
                local custom_dir="/etc/authselect/custom/prmana"
                if [ ! -d "$custom_dir" ]; then
                    authselect create-profile prmana --base-on "$current_profile" 2>/dev/null || {
                        log_warn "Could not create authselect profile — generating recommended configs only"
                        generate_pam_config
                        return 0
                    }
                fi
                log_success "Created authselect profile 'prmana'"
                log_info "Enable with: sudo authselect select custom/prmana"
...
install_service_files() {
    ...
    if [ -f "$src_dir/prmana-agent.service" ]; then
        cp "$src_dir/prmana-agent.service" "$user_unit_dir/"
        log_success "Installed $user_unit_dir/prmana-agent.service"
    fi

    if [ -f "$src_dir/prmana-agent.socket" ]; then
        cp "$src_dir/prmana-agent.socket" "$user_unit_dir/"
        log_success "Installed $user_unit_dir/prmana-agent.socket"
    fi

    # Install tmpfiles.d config (for system-level JTI/nonce directories)
    if [ -f "$src_dir/prmana.tmpfiles.conf" ]; then
        cp "$src_dir/prmana.tmpfiles.conf" /etc/tmpfiles.d/prmana.conf
        systemd-tmpfiles --create /etc/tmpfiles.d/prmana.conf 2>/dev/null || {
            log_warn "systemd-tmpfiles --create failed (directories may need manual creation)"
        }
        log_success "Installed /etc/tmpfiles.d/prmana.conf"
    fi

    # Reload systemd
    systemctl daemon-reload 2>/dev/null || true
    log_success "systemd units installed (enable with: systemctl --user enable --now prmana-agent.socket)"
}
...
usage() {
    cat << EOF
prmana installer v${SCRIPT_VERSION}

Usage: $0 [OPTIONS]

Options:
    --issuer URL              OIDC issuer URL (required for production)
    --client-id ID            OIDC client ID (default: prmana)
    --version VER             prmana version to install (default: $VERSION)
...
Examples:
    ...
    $0 --offline ./prmana-v${VERSION}-linux-x86_64.tar.gz --issuer https://login.example.com
...
main() {
    ...
    echo ""
    echo "==============================================================="
    echo "                 prmana Installer v${SCRIPT_VERSION}"
    echo "==============================================================="
    echo ""
    ...
    log_info "  - PAM module: $PAM_DIR/pam_prmana.so"
    if [ "$INSTALL_AGENT" = true ]; then
        log_info "  - Agent: /usr/local/bin/prmana-agent"
    fi
    log_info "  - Config: $INSTALL_DIR/"
    log_info "  - systemd units: /usr/lib/systemd/user/"
    log_info "  - tmpfiles: /etc/tmpfiles.d/prmana.conf"
    ...
    log_info "  4. Enable agent: systemctl --user enable --now prmana-agent.socket"
    if [ -z "$BREAKGLASS_USER" ]; then
