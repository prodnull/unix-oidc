...
//!
//! Run: cargo test -p prmana-agent --test step_up_ipc
...
async fn test_step_up_happy_path() {
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_prmana-agent"))
        ...
        .env("PRMANA_OIDC_ISSUER", &base_url)
        .env("PRMANA_OIDC_CLIENT_ID", "test-ciba-client")
        .env("PRMANA_OIDC_CLIENT_SECRET", "test-secret")
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .expect("start daemon");
...
// extraction) is covered by pam-prmana unit tests in src/sudo.rs. These
