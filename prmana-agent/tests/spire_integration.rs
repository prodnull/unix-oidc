...
fn test_spire_yaml_config_deserialization() {
    use prmana_agent::hardware::SpireYamlConfig;
...
fn test_signer_config_with_spire_section() {
    use prmana_agent::hardware::SignerConfig;
...
///
/// Run with: `cargo test -p prmana-agent --features spire -- --ignored spire`
/// Requires: SPIRE agent running with workload registered for this process.
/// Set `PRMANA_SPIRE_SOCKET` to override the default socket path.
#[tokio::test]
...
async fn test_spire_live_fetch_svid() {
    ...
    let config = SpireConfig {
        socket_path: socket,
        audience: vec!["prmana-test".to_string()],
        spiffe_id: None,
    };
...
async fn test_spire_live_svid_caching() {
    ...
    let config = SpireConfig {
        socket_path: socket,
        audience: vec!["prmana-cache-test".to_string()],
        spiffe_id: None,
    };
    let signer = SpireSigner::new(config).expect("SpireSigner::new in tokio runtime");
