...
//! `PRMANA_JTI_DIR`. Tests do not share any filesystem state (T-30-16).
//!
...
//!
//! Run: `cargo test -p prmana-agent --test jti_cross_fork`
...
///
/// `CARGO_BIN_EXE_prmana-jti-helper` is set by `cargo test` for every
/// `[[bin]]` entry in the same crate, so no manual path arithmetic is needed.
fn helper_bin() -> &'static str {
    env!("CARGO_BIN_EXE_prmana-jti-helper")
}
...
fn test_cross_fork_sequential_replay() {
    ...
    let status_a = Command::new(helper_bin())
        ...
        .env("PRMANA_JTI_DIR", jti_dir.path())
        .status()
...
fn test_concurrent_race_one_wins() {
    ...
    let mut child_c = Command::new(helper_bin())
        ...
        .env("PRMANA_JTI_DIR", &jti_dir_path)
        .spawn()
...
/// rejection (T-30-03).
///
/// This matches the production path: if `/run/prmana/jti` is misconfigured,
