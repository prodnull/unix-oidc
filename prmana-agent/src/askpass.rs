...
//! DPoP-bound SSH authentication (see `pam-prmana/src/lib.rs`):
//!
...
//!
//! - Nonce tmpfile: `$TMPDIR/.prmana-nonce-{PPID}`
//! - Token tmpfile: `$TMPDIR/.prmana-token-{PPID}`
//!
...
use prmana_agent::daemon::{AgentClient, AgentResponse, AgentResponseData};
...
fn nonce_tmpfile_path(ppid: u32) -> PathBuf {
    std::env::temp_dir().join(format!(".prmana-nonce-{ppid}"))
}
...
fn token_tmpfile_path(ppid: u32) -> PathBuf {
    std::env::temp_dir().join(format!(".prmana-token-{ppid}"))
}
...
pub async fn run_ssh_askpass(prompt: String) -> anyhow::Result<()> {
    ...
    if prompt.trim_start().starts_with("DPOP_PROOF") {
        ...
        let target = std::env::var("PRMANA_TARGET").unwrap_or_default();
...
mod tests {
    ...
    fn test_nonce_tmpfile_path_includes_ppid() {
        ...
        assert!(name.starts_with(".prmana-nonce-"));
    }
    ...
    fn test_token_tmpfile_path_includes_ppid() {
        ...
        assert!(name.starts_with(".prmana-token-"));
    }
    ...
    fn test_read_and_delete_returns_none_when_file_missing() {
        let path = std::env::temp_dir().join(".prmana-nonexistent-99999999");
    ...
    fn test_read_and_delete_returns_contents_and_removes_file() {
        let path = std::env::temp_dir().join(format!(".prmana-rad-test-{}", std::process::id()));
