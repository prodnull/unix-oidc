//! Metrics collection for prmana-agent
//!
...
impl MetricsSnapshot {
    ...
    pub fn to_prometheus(&self) -> String {
        ...
        // Uptime
        lines.push("# HELP prmana_agent_uptime_seconds Agent uptime in seconds".to_string());
        lines.push("# TYPE prmana_agent_uptime_seconds gauge".to_string());
        lines.push(format!(
            "prmana_agent_uptime_seconds {}",
            self.uptime_seconds
        ));

        // Proof requests
        lines.push("# HELP prmana_agent_proof_requests_total Total proof requests".to_string());
        lines.push("# TYPE prmana_agent_proof_requests_total counter".to_string());
        lines.push(format!(
            "prmana_agent_proof_requests_total{{status=\"success\"}} {}",
            self.proof_requests_success
        ));
        lines.push(format!(
            "prmana_agent_proof_requests_total{{status=\"failed\"}} {}",
            self.proof_requests_failed
        ));

        // Proof latency
        lines.push(
            "# HELP prmana_agent_proof_latency_us Proof generation latency in microseconds"
                .to_string(),
        );
        lines.push("# TYPE prmana_agent_proof_latency_us summary".to_string());
        lines.push(format!(
            "prmana_agent_proof_latency_us{{quantile=\"0.5\"}} {}",
            self.proof_latency_p50_us
        ));
        lines.push(format!(
            "prmana_agent_proof_latency_us{{quantile=\"0.95\"}} {}",
            self.proof_latency_p95_us
        ));
        lines.push(format!(
            "prmana_agent_proof_latency_us{{quantile=\"0.99\"}} {}",
            self.proof_latency_p99_us
        ));

        // Token refresh
        lines.push(
            "# HELP prmana_agent_token_refresh_total Total token refresh attempts".to_string(),
        );
        lines.push("# TYPE prmana_agent_token_refresh_total counter".to_string());
        lines.push(format!(
            "prmana_agent_token_refresh_total{{status=\"success\"}} {}",
            self.token_refresh_success
        ));
        lines.push(format!(
            "prmana_agent_token_refresh_total{{status=\"failed\"}} {}",
        ...
        lines.push("# HELP prmana_agent_ipc_requests_total Total IPC requests".to_string());
        lines.push("# TYPE prmana_agent_ipc_requests_total counter".to_string());
        lines.push(format!(
            "prmana_agent_ipc_requests_total {}",
            self.ipc_requests_total
        ));

        lines.push("# HELP prmana_agent_ipc_errors_total Total IPC errors".to_string());
        lines.push("# TYPE prmana_agent_ipc_errors_total counter".to_string());
        lines.push(format!(
...
mod tests {
    ...
    fn test_prometheus_format() {
        ...
        assert!(prom.contains("prmana_agent_uptime_seconds"));
        assert!(prom.contains("prmana_agent_proof_requests_total"));
    }
}
