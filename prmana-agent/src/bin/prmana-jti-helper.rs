...
//! `prmana-agent/tests/jti_cross_fork.rs`. It wraps `FsAtomicStore::check_and_record()`
//! and exits with codes the test harness can assert on.
//!
//! # Usage
//!
//! ```text
//! prmana-jti-helper check <issuer> <jti>
//! prmana-jti-helper check-permissive <issuer> <jti>
//! ```
//!
//! # Exit codes
//!
...
//! # Environment
//!
//! `PRMANA_JTI_DIR` — directory for the JTI filesystem store. **Required** for
//! test isolation; each test passes a unique `tempfile::tempdir()` path.

use pam_prmana::security::fs_store::{AtomicRecordResult, FsAtomicStore};
...
fn main() -> ExitCode {
    ...
    // FsAtomicStore reads PRMANA_JTI_DIR from the environment; each test
