...
use pam_prmana::identity::mapper::UsernameMapper;
use pam_prmana::oidc::jwks::{IssuerJwksRegistry, JwksProvider};
use pam_prmana::oidc::{TokenValidator, ValidationConfig};
...
fn entra_single_issuer_policy() -> PolicyConfig {
    ...
    policy.security_modes = Some(pam_prmana::policy::config::SecurityModes {
        jti_enforcement: EnforcementMode::Warn,
        ..Default::default()
    });
    policy
