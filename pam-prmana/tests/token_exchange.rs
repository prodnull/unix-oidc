...
use pam_prmana::oidc::token::{ActClaim, ConfirmationClaim, StringOrVec, TokenClaims};
use pam_prmana::oidc::validation::{
    validate_delegation, validate_delegation_optional, validate_exchanged_token_lifetime,
    ValidationError,
};
use pam_prmana::policy::config::DelegationConfig;
...
fn make_claims(sub: &str, iat: i64, exp: i64, act: Option<ActClaim>) -> TokenClaims {
    TokenClaims {
        ...
        aud: StringOrVec::String("prmana".into()),
        exp,
        iat,
        auth_time: None,
        acr: None,
