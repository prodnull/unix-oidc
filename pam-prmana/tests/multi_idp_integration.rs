...
//! Tests that need PRMANA_TEST_MODE set are serialized via ENV_MUTEX to prevent
...
use pam_prmana::auth::{authenticate_multi_issuer, AuthError, DPoPAuthConfig};
use pam_prmana::oidc::jwks::IssuerJwksRegistry;
...
use pam_prmana::security::nonce_cache::{
...
/// Serialise tests that set/remove PRMANA_TEST_MODE to prevent races with
...
/// skips signature verification when `PRMANA_TEST_MODE=1`. It is NOT a valid
...
fn make_test_token(iss: &str, sub: &str, preferred_username: &str, jti: Option<&str>) -> String {
    ...
    let payload = format!(
        r#"{{"iss":"{iss}","sub":"{sub}","aud":"prmana","exp":{exp},"iat":{now},"preferred_username":"{preferred_username}"{jti_field}}}"#
    );
...
fn make_test_token_with_acr(
    ...
    let payload = format!(
        r#"{{"iss":"{iss}","sub":"{sub}","aud":"prmana","exp":{exp},"iat":{now},"preferred_username":"{preferred_username}"{jti_field}{acr_field}}}"#
    );
    let h = URL_SAFE_NO_PAD.encode(header.as_bytes());
    let p = URL_SAFE_NO_PAD.encode(payload.as_bytes());
    format!("{h}.{p}.dummysig")
...
fn test_jti_same_value_different_issuers_no_collision() {
    use pam_prmana::security::jti_cache::global_jti_cache;
...
"#;
    ...
    assert_eq!(issuer.client_id, "prmana-min");
...
"#;
    ...
    assert_eq!(
        ...
        Some("api://prmana".to_string()),
        "expected_audience must deserialise as Some(\"api://prmana\")"
    );
...
fn test_health_state_degraded_after_three_failures() {
    ...
    std::env::set_var("PRMANA_HEALTH_DIR", dir.path().to_str().unwrap());
    ...
    std::env::remove_var("PRMANA_HEALTH_DIR");
}
...
fn test_config_fresh_detects_changed_mtime() {
    ...
    std::env::set_var("PRMANA_POLICY", config_path.to_str().unwrap());
    ...
    std::env::remove_var("PRMANA_POLICY");
}
