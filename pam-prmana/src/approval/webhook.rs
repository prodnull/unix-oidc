...
impl WebhookConfig {
    ...
    /// - `PRMANA_WEBHOOK_URL` (required): The webhook URL
    /// - `PRMANA_WEBHOOK_AUTH` (optional): Authorization header value
    /// - `PRMANA_WEBHOOK_TIMEOUT` (optional): Request timeout in seconds
    /// - `PRMANA_WEBHOOK_INSECURE` (optional): Set to "true" to disable TLS verification
    pub fn from_env() -> Result<Self, ApprovalError> {
        let webhook_url = std::env::var("PRMANA_WEBHOOK_URL")
            .map_err(|_| ApprovalError::ConfigError("PRMANA_WEBHOOK_URL not set".to_string()))?;

        let mut config = Self::new(&webhook_url);

        if let Ok(auth) = std::env::var("PRMANA_WEBHOOK_AUTH") {
            config = config.with_auth(&auth);
        }

        if let Ok(timeout_str) = std::env::var("PRMANA_WEBHOOK_TIMEOUT") {
            if let Ok(timeout) = timeout_str.parse() {
                config = config.with_timeout(timeout);
            }
        }

        if let Ok(secret) = std::env::var("PRMANA_WEBHOOK_HMAC_SECRET") {
            if !secret.is_empty() {
        ...
        #[cfg(feature = "test-mode")]
        if std::env::var("PRMANA_WEBHOOK_INSECURE")
...
impl WebhookApprovalProvider {
    ...
    fn build_request(
        ...
        builder = builder.header("User-Agent", "prmana/0.1");
...
mod tests {
    ...
    fn test_webhook_config_default_tls_enabled() {
        ...
        // Set only the required env var; leave PRMANA_WEBHOOK_INSECURE unset.
        let _url_guard = TempEnvGuard::set("PRMANA_WEBHOOK_URL", "https://example.com/hook");
        let _insecure_guard = TempEnvGuard::remove("PRMANA_WEBHOOK_INSECURE");
        let _hmac_guard = TempEnvGuard::remove("PRMANA_WEBHOOK_HMAC_SECRET");

        let config = WebhookConfig::from_env().unwrap();
        assert!(
            config.verify_tls,
            "verify_tls must default to true when PRMANA_WEBHOOK_INSECURE is unset"
        );
    }

    /// F-06 negative: in non-test-mode builds, setting PRMANA_WEBHOOK_INSECURE=true
    ...
    fn test_webhook_insecure_env_ignored_without_test_mode() {
        let _lock = ENV_MUTEX.lock().unwrap();
        let _url_guard = TempEnvGuard::set("PRMANA_WEBHOOK_URL", "https://example.com/hook");
        let _insecure_guard = TempEnvGuard::set("PRMANA_WEBHOOK_INSECURE", "true");
        let _hmac_guard = TempEnvGuard::remove("PRMANA_WEBHOOK_HMAC_SECRET");

        let config = WebhookConfig::from_env().unwrap();
        assert!(
            config.verify_tls,
            "verify_tls must remain true even when PRMANA_WEBHOOK_INSECURE=true \
             in non-test-mode builds"
        );
    }

    /// F-06 test-mode: setting PRMANA_WEBHOOK_INSECURE=true disables TLS verification.
    #[cfg(feature = "test-mode")]
    #[test]
    fn test_webhook_insecure_env_disables_tls_in_test_mode() {
        ...
        assert!(
            !config.verify_tls,
            "verify_tls must be false when PRMANA_WEBHOOK_INSECURE=true in test-mode"
        );
    }
    ...
    fn test_hmac_secret_from_env() {
        ...
        let _url_guard = TempEnvGuard::set("PRMANA_WEBHOOK_URL", "https://example.com/hook");
        let _insecure_guard = TempEnvGuard::remove("PRMANA_WEBHOOK_INSECURE");
        let _hmac_guard = TempEnvGuard::set("PRMANA_WEBHOOK_HMAC_SECRET", "env-secret");
    ...
    fn test_hmac_secret_empty_env_ignored() {
        ...
        let _url_guard = TempEnvGuard::set("PRMANA_WEBHOOK_URL", "https://example.com/hook");
        let _insecure_guard = TempEnvGuard::remove("PRMANA_WEBHOOK_INSECURE");
        let _hmac_guard = TempEnvGuard::set("PRMANA_WEBHOOK_HMAC_SECRET", "");
