...
//! - `PRMANA_RATE_LIMIT_WINDOW`: Window size in seconds (default: 300)
//! - `PRMANA_RATE_LIMIT_MAX_ATTEMPTS`: Max attempts per window (default: 5)
//! - `PRMANA_RATE_LIMIT_LOCKOUT`: Initial lockout in seconds (default: 60)
...
impl RateLimitConfig {
    ...
    pub fn from_env() -> Self {
        let window = std::env::var("PRMANA_RATE_LIMIT_WINDOW")
            .ok()
            .and_then(|s| s.parse().ok())
            .map(Duration::from_secs)
            .unwrap_or(Duration::from_secs(DEFAULT_WINDOW_SECS));

        let max_attempts = std::env::var("PRMANA_RATE_LIMIT_MAX_ATTEMPTS")
            .ok()
            ...
            .map(|v| {
                ...
                if clamped != v {
                    tracing::warn!(
                        ...
                        "PRMANA_RATE_LIMIT_MAX_ATTEMPTS out of valid range, clamped"
                    );
                }
                clamped
            })
        ...
        let initial_lockout = std::env::var("PRMANA_RATE_LIMIT_LOCKOUT")
            .ok()
            .and_then(|s| s.parse().ok())
            .map(Duration::from_secs)
...
mod tests {
    ...
    fn test_rate_limit_config_valid_max_attempts() {
        std::env::set_var("PRMANA_RATE_LIMIT_MAX_ATTEMPTS", "50");
        let config = RateLimitConfig::from_env();
        std::env::remove_var("PRMANA_RATE_LIMIT_MAX_ATTEMPTS");
    ...
    fn test_rate_limit_config_zero_clamped_to_min() {
        std::env::set_var("PRMANA_RATE_LIMIT_MAX_ATTEMPTS", "0");
        let config = RateLimitConfig::from_env();
        std::env::remove_var("PRMANA_RATE_LIMIT_MAX_ATTEMPTS");
        assert_eq!(
    ...
    fn test_rate_limit_config_large_clamped_to_max() {
        std::env::set_var("PRMANA_RATE_LIMIT_MAX_ATTEMPTS", "999999");
        let config = RateLimitConfig::from_env();
        std::env::remove_var("PRMANA_RATE_LIMIT_MAX_ATTEMPTS");
        assert_eq!(
