...
///
/// ```
/// use pam_prmana::security::JtiCache;
///
...
/// Global filesystem-based JTI store. Directory: `/run/prmana/jti/`.
///
...
///
/// Override the backing directory with `PRMANA_JTI_DIR` for testing.
pub fn global_jti_store() -> &'static FsAtomicStore {
    ...
    // /run/prmana/jti (which doesn't exist on dev machines), causing all
    ...
    static STORE: Lazy<FsAtomicStore> =
        Lazy::new(|| FsAtomicStore::new("/run/prmana/jti", "PRMANA_JTI_DIR"));
    &STORE
}

/// Ensure `PRMANA_JTI_DIR` points to a per-process tempdir before the
/// `Lazy<FsAtomicStore>` in `global_jti_store()` is first accessed.
///
/// All test modules (auth, dpop, etc.) MUST call this single function
/// instead of setting `PRMANA_JTI_DIR` independently. The `Lazy` reads
...
pub fn setup_test_jti_dir() {
    ...
    TEST_JTI_DIR.get_or_init(|| {
        ...
        let dir = std::env::temp_dir().join(format!("prmana-jti-test-{}", std::process::id()));
        #[allow(clippy::expect_used)]
        std::fs::create_dir_all(&dir).expect("create JTI test tempdir");
        std::env::set_var("PRMANA_JTI_DIR", &dir);
        dir
    });
}
...
///   without the `/run/prmana/` tmpfiles directory. A **LOG_CRIT** message
        ...
        AtomicRecordResult::IoError(e) => match enforcement {
            ...
            EnforcementMode::Warn | EnforcementMode::Disabled => {
                ...
                emit_syslog_crit(&format!(
                    "prmana: JTI filesystem store unavailable at /run/prmana/jti/ - \
