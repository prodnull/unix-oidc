...
//!   `systemd-tmpfiles` — see `contrib/systemd/prmana.tmpfiles.conf`).
...
///
/// ```rust,no_run
/// use pam_prmana::security::FsAtomicStore;
///
/// let store = FsAtomicStore::new("/run/prmana/jti", "PRMANA_JTI_DIR");
...
impl FsAtomicStore {
    ...
    /// # Arguments
    ///
    /// * `default_dir` — Default directory path (e.g. `/run/prmana/jti`).
...
mod tests {
    ...
    fn test_check_and_record_new_key_returns_new() {
        ...
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_01");
    ...
    fn test_check_and_record_same_key_returns_already_exists() {
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_02");
        let expires = now_unix() + 300;
    ...
    fn test_check_and_record_same_value_different_scope_returns_new() {
        ...
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_03");
        let expires = now_unix() + 300;
    ...
    fn test_consume_after_check_and_record_returns_true() {
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_04");
        let expires = now_unix() + 300;
    ...
    fn test_consume_twice_returns_false_second_time() {
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_05");
        let expires = now_unix() + 300;
    ...
    fn test_consume_never_created_returns_false() {
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_06");
    ...
    fn test_check_and_record_unwritable_directory_returns_io_error() {
        ...
        let store = FsAtomicStore {
            dir: ro_dir.clone(),
            env_var: "PRMANA_TEST_FS_07_UNUSED",
        };
    ...
    fn test_key_path_produces_64_char_hex() {
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_08");
    ...
    fn test_sweep_expired_deletes_expired_keeps_fresh() {
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_09");
    ...
    fn test_file_contents_contain_expires_at_timestamp() {
        let tmp = tempdir().unwrap();
        let store = make_store(tmp.path(), "PRMANA_TEST_FS_10");
